...
void loadCognitiveRequestQueue(std::string fileName, std::streampos & filePosition, CognitiveRequestQueue & cognitiveRequestQueue, AgentList & agentList, unsigned long long int interval)
{
   ...
   /*debug*/std::cout << "[ LOAD COG REQ   ]" <<  std::endl;

   /*debug*/std::cout << "[" << interval  << "][" << " interval " <<  "]" <<  std::endl;
   /*debug*/std::cout << "[" << request.interval  << "][" << " interval on file" <<  "]" <<  std::endl;
   ...
   while (interval == request.interval and !file.eof())
   {
      ...
      ///*debug*/std::cout << "[" << interval  << "][" << " interval " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << request.minimumThroughput  << "][" << " Throughput " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << request.deltaInterval  << "][" << " Delta Interval " <<  "]" <<  std::endl;
...
char * writeCognitiveRequestQueue(key_t memoryKey, char * sharedMemory, CognitiveRequestQueue & cognitiveRequestQueue, unsigned long long int interval)
{
   ...
   memcpy(sharedMemory, writeOn.str().data(), writeOn.str().size());
   sharedMemory += writeOn.str().size();
   ///*debug*/std::cout << "[" << cognitiveRequestQueue.request.size()  << "][" << " cognitiveRequestQueue.request.size() " <<  "]" <<  std::endl;
   /*debug*/std::cout << "[" << interval  << "][" << " interval " <<  "]" <<  std::endl;
   ///*debug*/std::cout << "[" << cognitiveRequestQueue.request.at(0).interval << "][" << " cognitiveRequestQueue.request.at(cognitiveRequestIndex).interval " <<  "]" <<  std::endl;
   ...
   for (unsigned int cognitiveRequestIndex = 0; cognitiveRequestIndex < cognitiveRequestQueue.request.size() and interval == cognitiveRequestQueue.request.at(cognitiveRequestIndex).interval; cognitiveRequestIndex++)
   {
      ...
      memcpy(sharedMemory, writeOn.str().data(), writeOn.str().size());
      sharedMemory += writeOn.str().size();
      ///*debug*/std::cout << "[" << writeOn.str()  << "][" << " request inside for  " <<  "]" <<  std::endl;

   }

   //end tag
   /*debug*/std::cout << "[" << writeOn.str()  << "][" << " Writing on the cog requests: last one  " <<  "]" <<  std::endl;
   memcpy(sharedMemory, writeOn.str().data(), writeOn.str().size());
   ...
   memcpy(sharedMemory, writeOn.str().data(), writeOn.str().size());
   sharedMemory += writeOn.str().size();
   ///*debug*/std::cout << "[" << writeOn.str()  << "][" << " end of request in one interval  " <<  "]" <<  std::endl;
...
char * readCognitiveQueue(key_t memoryKey, char * sharedMemory, CognitiveQueue & cognitiveQueue, AgentList * agentList, unsigned long long int interval)
{
   ...
   if (tag.str().compare(parser.substr(0,TAG_SIZE)) == 0)
   {
      ...
      ///*debug*/std::cout << "[" << readFrom[0]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[1]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[2]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[3]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[4]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[5]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[6]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[7]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[17]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[18]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[19]  << "][" << " read from inside if " <<  "]" <<  std::endl;
      ///*debug*/std::cout << "[" << readFrom[20]  << "][" << " read from inside if " <<  "]" <<  std::endl;

      parser = readFrom;

      /*debug*/ std::cout << "[" << tag.str() << "][" << " tag " << "]" << std::endl;
      /*debug*/ std::cout << "[" << parser.substr(0,TAG_SIZE) << "][" << " read tag " << "]" << std::endl;
      /*debug*/ std::cout << "[" << (tag.str().compare(parser.substr(0,TAG_SIZE)) != 0) << "  compare tag " << std::endl;

      while (tag.str().compare(parser.substr(0,TAG_SIZE)) != 0)
      {
         /*debug*/ std::cout << " [ inside while ]" << std::endl;
         //Aline
         memcpy(&localInterval, &readFrom[0], INTERVAL_SIZE);
         ///*debug*/std::cout << "[" << localInterval[0]  << "][" << " local interval 0 " <<  "]" <<  std::endl;
         ///*debug*/std::cout << "[" << localInterval[1]  << "][" << " local interval 1 " <<  "]" <<  std::endl;
         ///*debug*/std::cout << "[" << localInterval[2]  << "][" << " local interval 2 " <<  "]" <<  std::endl;
         ///*debug*/std::cout << "[" << localInterval[3]  << "][" << " local interval 3 " <<  "]" <<  std::endl;
         ///*debug*/std::cout << "[" << localInterval[4]  << "][" << " local interval 4 " <<  "]" <<  std::endl;
         ///*debug*/std::cout << "[" << localInterval[5]  << "][" << " local interval 5 " <<  "]" <<  std::endl;
         ///*debug*/std::cout << "[" << localInterval[6]  << "][" << " local interval 6 " <<  "]" <<  std::endl;

         event.interval = byteToInt(localInterval, INTERVAL_SIZE);

         /*debug*/ std::cout << " [ " << event.interval << " ][ " << " interval (int) " << " ] " << std::endl;
         /*debug*/ std::cout << " [ " <<interval << " ][ " << " waited interval (int) " << " ] " << std::endl;
         ...
         if (interval == event.interval)
         {
            ...
            /*debug*/ std::cout << " [ " << transmitterType << " ][ " << " transmitter  " << " ] " << std::endl;
            /*debug*/ std::cout << " [ " << AgentTypeChar[CA] << " ][ " << " expected transmitter   " << " ] " << std::endl;
            ...
            if (strcmp(transmitterType, AgentTypeChar[CA]) == 0)
            {
               ...
               /*debug*/ std::cout << " [ " << event.transmitter<< " ][ " << " event transmitter" << " ] " << std::endl;

               memcpy(&receiverType, &readFrom[12], AGENT_TYPE_SIZE);
               /*debug*/ std::cout << " [ " << receiverType << " ][ " << " receiver " << " ] " << std::endl;
               /*debug*/ std::cout << " [ " << AgentTypeChar[CC] << " ][ " << " expected receiver " << " ] " << std::endl;
               ...
               if (strcmp(receiverType, AgentTypeChar[CC]) == 0)
               {
                  ...
                  /*debug*/ std::cout << " [ " << event.channel << " ][ " << " channel (int)  " << " ] " << std::endl;
   ...
   /*debug*/std::cout << " Returning " << std::endl;
...
void cognitiveSuccessQueueCalculus(SuccessQueue & cognitiveSuccessQueue, LicensedQueue & licensedQueue, CognitiveQueue & cognitiveQueue, unsigned long long int interval, WirelessParameters * wirelessParametersList)
{
   ...
   /*debug*/std::cout << "[" << cognitiveQueue.events.size()  << "][" << " cognitiveQueue.events.size() " <<  "]" <<  std::endl;
   ///*debug*/std::cout << "[" << cognitiveQueue.events.at(cognitiveEventIndexA).interval;  << "][" << " cog interval  " <<  "]" <<  std::endl;
   /*debug*/std::cout << "[" << interval  << "][" << " interval  " <<  "]" <<  std::endl;
   ...
   for (unsigned int cognitiveEventIndexA = 0; cognitiveEventIndexA < cognitiveQueue.events.size() and interval == cognitiveQueue.events.at(cognitiveEventIndexA).interval; cognitiveEventIndexA++)
   {
     /*debug*/std::cout << "[" << cognitiveQueue.events.at(cognitiveEventIndexA).interval << "][" << " cog interval  " <<  "]" <<  std::endl;
      ...
      //Interference from Licensed Network
      /*debug*/std::cout << "[" << licensedQueue.events.size() << "][" << " licensedQueue.events.size()  " <<  "]" <<  std::endl;
      ...
      {
        /*debug*/std::cout << "[" << licensedQueue.events.at(licensedEventIndexB).interval << "][" << " licensedQueue.events.at(licensedEventIndexB).interval  no terceiro for " <<  "]" <<  std::endl;

         transmitterB = licensedQueue.events.at(licensedEventIndexB).transmitter;
         receiverB = licensedQueue.events.at(licensedEventIndexB).receiver;

         /*debug*/std::cout << "[" << transmitterB->type << "][" << " transmitterB type dentro do 3o for " <<  "]" <<  std::endl;
         /*debug*/std::cout << "[" << receiverB->type << "][" << " receiverB type dentro do 3o for  " <<  "]" <<  std::endl;

         //if it is a license passive communication (unidirectional, from antenna to client), then ignore it since there is always a beacon on the same channel that will be take into account (receiver = invalid)
         if ((transmitterB->type == AT and receiverB->type != IV) or (transmitterB->type == AH and (receiverB->type != IV or receiverB->type != AC))) continue;
         channelB = licensedQueue.events.at(licensedEventIndexB).channel;
         /*debug*/std::cout << "[" << channelB << "][" << " channel do 3o for  " <<  "]" <<  std::endl;
      ...
      ///*debug*/std::cout << "[" << cognitiveQueue.events.at(cognitiveEventIndexA) << "][" << " cognitiveQueue.events.at(cognitiveEventIndexA) fim do 3o for " <<  "]" <<  std::endl;
      /*debug*/std::cout << "[" << success.success << "][" << " success fim do 3o for  " <<  "]" <<  std::endl;
