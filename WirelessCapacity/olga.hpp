...
int olga(int argn, char ** argc)
{
	...
	//	0.030	20000000	4.0			25		100		~/git/general/WISp/networks/100/nodes/graph.1000.0.dot.node
	...
	for (unsigned long int newNode = 0; newNode < numberOfNodes; newNode++)
	{
		for (unsigned long int node = newNode +1; node  < numberOfNodes; node++)
			...
			{
				neighborhood.at(newNode).at(node) = false;
				neighborhood.at(node).at(newNode) = false;
			}
	}
	...
	//olga---
	long double olgaDistance;
	long double olgaInverseSNRLimit = 2/(long double)3;
	long double olgaC = std::pow(std::pow(2,5) * std::pow(3,2) * dBtoSNR(snrThreshold) * (pathLoss-1)/(pathLoss-2) , 1/pathLoss);
	if (olgaC < 2) olgaC = 2;
	...
	/*debug*/
	//printLinks(links, snrThreshold, bandWidth);
	/*debug*/

	//links must be order by link power in the receiver
	for (unsigned long int link = 1; link < links.size(); link++)
	{
		linkToInsert = links.at(link);
		holePosition = link;
		while (holePosition > 0 and linkToInsert.powerReceived > links.at(holePosition - 1).powerReceived)
		{
	...
	if (links.size() == 0)
	{
		printResults("olga", fileName.str(), 0, numberOfNodes, slots.size(),links.size(), false);
		return -1;
	}
	...
	do
	{
		...
		while (emptySlot.candidates.size() > 0)
		{
			...
			/*debug*/std::cout << "co" << links.at(0).numberOfSchedules << "/" << emptySlot.candidates.size() << "sl" << currentSlot << "/" << slots.size() << " ";
			...
			while (slots.at(currentSlot).candidates.size() > 0)
			{
				...
				for (unsigned long int scheduledIndex = 0; scheduledIndex < slots.at(currentSlot).scheduled.size(); scheduledIndex++)
				{
					...
					scheduledDistance = euclideanDistance(nodePosition.at(links.at(scheduledLink).senderNode), nodePosition.at(links.at(scheduledLink).receiverNode));
					olgaDistance = euclideanDistance(nodePosition.at(links.at(candidateLink).senderNode), nodePosition.at(links.at(scheduledLink).receiverNode));

					if (olgaDistance <= (olgaC * scheduledDistance) or (candidateInverseSNR * dBtoSNR(snrThreshold)) >= olgaInverseSNRLimit)
					{
						candidateOk = false;
						break;
					}

					if ((links.at(scheduledLink).interferedBy.at(candidateLink) / links.at(scheduledLink).interferedBy.at(scheduledLink)) > 1/dBtoSNR(snrThreshold))
					{candidateOk = false; break;}
					if (slots.at(currentSlot).inverseSNR.at(scheduledIndex) > 1/dBtoSNR(snrThreshold))
					{candidateOk = false; break;}
					//Sanity check???
				...
				//eliminate the candidate link from the current slot because every link cannot be schedule twice in the same slot
	...
	//results
	printResults("olga", fileName.str(), bestNumberOfschedules, numberOfNodes, bestSlotsSize, links.size(), false);
