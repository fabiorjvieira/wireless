...
char * readCognitiveQueue(key_t memoryKey, char * sharedMemory, CognitiveQueue & cognitiveQueue, AgentList * agentList, unsigned long long int interval)
{
   ...
   char transmitterID[AGENT_TYPE_SIZE];
...
void saveLicensedEventsQueue(std::string fileName, std::vector < Agent > cognitiveAntennas, std::vector < Agent > & cognitiveClients, std::vector < Agent > & licensePassiveClients, unsigned long long int lastInterval, double intervalExpectedValue, double deltaIntervalExpectedValue)
{
   std::ofstream file;
   unsigned int cognitiveAntennaIndex, cognitiveClientIndex;
